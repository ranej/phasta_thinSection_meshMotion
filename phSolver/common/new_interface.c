...
Write_Restart_Stagger(  int* pid,
    ...
    if(outpar.output_mode == -1 )
      ...
      exit(EXIT_FAILURE);
    phio_constructName(f_descriptor,"restart_stagger",filename);
    phstr_appendInt(filename, *stepno);
    phstr_appendStr(filename, ".");
    phstr_appendInt(filename, *lstagger);
    phstr_appendStr(filename, ".");
//    phstr_appendInt(filename, *lstagger);
    phio_openfile(filename, f_descriptor);

    printf("Debug Rane lstagger is HERE: %d \n", lstagger);
    field_flag=0;
    ...
    iotime = TMRC() - iotime;
    if (workfc.master == workfc.myrank)
      printf("time to write restart_stagger (seconds) %f\n",iotime);
}
