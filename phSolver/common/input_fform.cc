...
// read rigid body parameters, properties and constraints ---------------
    ...
    for (i = 0; i < rigidbody.numrbs; ++i){
      ...
      else if (RBstr == "Translation-only-prescribed") {
        rigidbody.rbsMM[i] = 3;
        cout << "Translation-only-prescribed set" << endl;
      }
    ...
    ivec = inp.GetValue("Rigid Body Prescribed Case");
    for (i = 0; i < rigidbody.numrbs; ++i)
      rigidbody.rbsPcase[i] = ivec[i];
      cout << ivec[i] << ":prescribed case" << endl;
    ivec.erase(ivec.begin(),ivec.end());
...
// read rotating band face tags ---------------
    rotatingband.numRotBands = inp.GetValue("Number of Rotating Bands");
    if (rotatingband.numRotBands > MAXTS) {
      cout << "ERROR in Input: increase MAXTS and recompile\n";
      exit(1);
    }

    str0.assign("Number of Face Tags for Rotating Band ");
    for (i = 0; i < rotatingband.numRotBand; ++i) {
      string str;
      stringstream ss;
      ss << i;
      str = str0 + ss.str();
      rotatingband.numFaceTags = inp.GetValue(str);
      str1.assing("Face Tags for Rotating Band ");
      ss << i;
      str = str1 + ss.str();
      ivec = inp.GetValue(str);
      for(j = 1; j < numFaceTags; ++j)
